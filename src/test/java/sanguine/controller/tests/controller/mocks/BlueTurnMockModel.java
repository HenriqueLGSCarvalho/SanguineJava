 ...
 *
 * <p>Its main purpose is to maintain a state where it is always blue's turn,
 * so it is possible to see how the controller will delegate to the model during that state.
 */
public class BlueTurnMockModel implements SanguineModel {

  private final Appendable log;

  /**
   * Represents an instance of the mock model for blue's turn.
   *
   * @param log the log which contains all the method calls from this class
   */
  public BlueTurnMockModel(Appendable log) {
    this.log = log;
  }
  ...
  public void passTurn() throws IllegalStateException {
    try {
      log.append("Blue turn passed." + System.lineSeparator());
    } catch (IOException e) {
      throw new RuntimeException();
    }
  }
  ...
  public List<Card> getBlueHand() throws IllegalStateException {
    Card testBlueCard1 = Card.builder()
            .name("TestBlueCard1")
            .value(1)
            .cost(1)
            .influenceGrid(new String[]{""}).build();

    Card testBlueCard2 = Card.builder()
            .name("TestBlueCard2")
            .value(1)
            .cost(1)
            .influenceGrid(new String[]{""}).build();

    return List.of(testBlueCard1, testBlueCard2);
  }
