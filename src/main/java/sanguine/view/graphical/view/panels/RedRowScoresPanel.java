 ...
 * If red's row score > blue's row score then the row score for that row will be highlighted red
 */
public class RedRowScoresPanel extends JPanel {
   ...
   */
  public RedRowScoresPanel(ReadOnlySanguineModel model) {
    this.model = model;
  }
  ...
  protected void paintComponent(Graphics g) {
    ...
    for (int row = 0; row < modelNumRow; row++) {
      drawScore(g2d, row, model.getRedRowScores()[row]);
    }
  }

  // Draws each rows score for the red Player
