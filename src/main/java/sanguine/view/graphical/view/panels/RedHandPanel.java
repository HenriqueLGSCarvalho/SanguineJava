...
/**
 * This class represents the panel for the red Player's hand of card.
 * Fully uncorrelated with the panel for the blue Player's hand.
 */
public class  RedHandPanel extends JPanel {
  ...
  /**
   * This represents an instance of red's hand panel.
   *
   * @param model the model of a game of sanguine.Sanguine
   */
  public RedHandPanel(ReadOnlySanguineModel model) {
    if (model == null) {
  ...
  protected void paintComponent(Graphics g) {
    ...
    int handSize = model.getRedHand().size();
    ...
    for (int cardIndex = 0; cardIndex < handSize; cardIndex++) {
      drawCardInfo(g2d, cardIndex,
              model.getRedHand().get(cardIndex).name(),
              "cost: " + model.getRedHand().get(cardIndex).cost(),
              "value: " + model.getRedHand().get(cardIndex).value(),
              model.getRedHand().get(cardIndex).influenceGrid()[0],
              model.getRedHand().get(cardIndex).influenceGrid()[1],
              model.getRedHand().get(cardIndex).influenceGrid()[2],
              model.getRedHand().get(cardIndex).influenceGrid()[3],
              model.getRedHand().get(cardIndex).influenceGrid()[4]);
    }
  ...
  private void highlightClickedCard(Graphics2D g2d) {
    int numCards = model.getRedHand().size();
  ...
  public void addListener(FeaturesListener listener) {
    this.addMouseListener(new MouseClickListener(listener, SanguineViewRedFrame.Clicks.RED_CARD,
            this, model));
  }
...
TODO:
  - Draw actual cards
  - place actual card in the correct index
  - highlight actual card
  - create a new frame for blueHand and copy and paste
    - Copy RedHandPanel and change it to blueHand stuff
    - Rename SanguineViewFrame to SanguineViewRedFrame
    - Make SanguineViewBlueFrame and copy all of SanguineViewRedFrame besides the redFrame
